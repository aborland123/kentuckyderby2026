...
.ht thead th {
    ...
    letter-spacing:0.07em; color:#555; padding:5px 8px;
    border-bottom:1px solid #2a2a2a; text-align:left;
}
.ht tbody td { padding:7px 8px; border-bottom:1px solid #1e1e1e; }
.ht .hit  { background:#0d1f14; }
.ht .miss { background:#0f0f0f; }
.h-yes { color:#22c55e; font-size:16px; font-weight:700; }
.h-no  { color:#2a2a2a; font-size:16px; }
...
HOLDOUT = [
    (2019,"Country House","Country House",True, 1),
    (2020,"Authentic",    "Authentic",    True, 1),
    (2021,"Mandaloun",    "Mandaloun",    True, 1),
    (2022,"Rich Strike",  "Rich Strike",  True, 1),
    (2023,"Tapit Trice",  "Mage",         False,2),
    (2024,"Sierra Leone", "Mystik Dan",   False,7),
    (2025,"Sandman",      "Sovereignty",  False,2),
]
...
def run_model(track_cond="Fast", scratched_pps=(9,)):   # tuple so it's hashable / cacheable
    try:
    ...
    def lk(d, n, s, dflt):
        if not n: return dflt
        nl = str(n).lower()
        for k, v in d.items():
            if str(k).lower() in nl or nl in str(k).lower(): return v[s]
        return dflt
    ...
    F = ["pp","ppw","ppt","ppc","ppi","ppo","tf","ts","jw","jt","tw2","tt","ts2",
         "ip","lo","fav","shot","orr","fs","ct","cw","cp","jtc",
         "beyer","bey100","bey104","beysub97"]
    ...
    for pp_n,horse,odds,best_bey,last_bey,prev_bey,prep,prep_date,cnt,note,hardcoded_scr in FIELD:
        ...
        rows.append({
            ...
            # display cols — use distinct names to avoid pandas method collisions
...
# LOAD
# ══════════════════════════════════════════════════════════════════════════════
df26, stats, err = run_model(track_cond, all_scratched_pps)
...
kpi(k1,"Holdout Accuracy","4 / 7","57% on 2019–2025")
kpi(k2,"Test AUC","0.856","was 0.555 without Beyer")
kpi(k3,"Avg Winner Rank","2.1","was 9.1 without Beyer")
kpi(k4,"100+ Beyer Rule","69%","of winners since 2000")
n_active = len(df26)
n_qual   = int((df26["best_beyer"]>=100).sum())
kpi(k5,"Field Qualified",f"{n_qual} / {n_active}","horses with 100+ Beyer")
...
# WIN / PLACE / SHOW
# ══════════════════════════════════════════════════════════════════════════════
st.markdown('<div class="sec-title">Model Picks</div>', unsafe_allow_html=True)
...
with right:
    ...
    # Holdout table
    st.markdown('<div class="sec-title">2019–2025 Holdout Results</div>',
                unsafe_allow_html=True)
    h_html = '<table class="ht"><thead><tr><th>Year</th><th>Model Pick</th>'
    h_html += '<th>Actual Winner</th><th style="text-align:right;">Rank</th><th></th></tr></thead><tbody>'
    for yr, pick, actual, hit, rank in HOLDOUT:
        rc  = "hit" if hit else "miss"
        sym = '<span class="h-yes">✓</span>' if hit else '<span class="h-no">○</span>'
        rk_s = f'<span style="color:#3b82f6;font-weight:700;">#1</span>' if rank==1 else f'<span style="color:#555;">#{rank}</span>'
        pw   = 'font-weight:600;color:#f0ede6;' if hit else 'color:#888;'
        h_html += (f'<tr class="{rc}">'
                   f'<td style="color:#555;font-weight:500;">{yr}</td>'
                   f'<td style="{pw}">{pick}</td>'
                   f'<td style="color:{"#e8e6e0" if hit else "#555"};">{actual}</td>'
                   f'<td style="text-align:right;">{rk_s}</td>'
                   f'<td style="text-align:center;">{sym}</td></tr>')
    h_html += "</tbody></table>"
    st.markdown(h_html, unsafe_allow_html=True)
    st.markdown(
        "<div style='font-size:11px;color:#444;margin-top:5px;margin-bottom:1.25rem;'>"
        f"4/7 correct (57%) &nbsp;·&nbsp; Avg winner rank: 2.1 &nbsp;·&nbsp; "
        f"AUC: {stats['auc']:.3f} &nbsp;·&nbsp; Random baseline: ~1/7 (14%)"
        "</div>", unsafe_allow_html=True)
